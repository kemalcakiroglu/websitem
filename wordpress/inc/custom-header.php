 ...
 *
 * @uses blogslog_header_style()
 */
function blogslog_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'blogslog_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'ff696a',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'blogslog_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'blogslog' ),
		),
	) );
}
add_action( 'after_setup_theme', 'blogslog_custom_header_setup' );

if ( ! function_exists( 'blogslog_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see blogslog_custom_header_setup().
	 */
	function blogslog_header_style() {
		...
		wp_add_inline_style( 'blogslog-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'blogslog_header_style', 10 );